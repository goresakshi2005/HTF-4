...
def build_fix_proposal_from_patch(
    ...
    # If the patch is a full JSON object (whole file content), handle it specially
    patch_text = patch_result.patch.strip()
    if patch_text.startswith('{') and patch_text.endswith('}'):
        # This is a full file replacement for a JSON file
        if len(patch_result.touched_files) != 1:
            raise ValueError("Expected exactly one touched file for full JSON replacement")
        path = patch_result.touched_files[0]
        if path not in file_contents:
            raise ValueError(f"Patch references unavailable file: {path}")
        # Validate JSON
        try:
            json.loads(patch_text)
        except json.JSONDecodeError as e:
            raise ValueError(f"Invalid JSON provided by LLM: {e.msg} at line {e.lineno} col {e.colno}") from e
        return CodeFixProposal(
            title="ci: llm auto-repair patch",
            description=patch_result.rationale,
            file_changes=[FileChange(path=path, content=patch_text)],
        )

    # Otherwise, treat it as a unified diff
    ...
    for parsed in parsed_files:
        ...
        if parsed.path.endswith('.json'):
            try:
                ...
                raise ValueError(f"Invalid JSON after patch: {e.msg} at line {e.lineno} col {e.colno}") from e
